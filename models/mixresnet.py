...
class ResNet(nn.Module):
    def __init__(
        self, block, conv_func, layers, num_classes=1000, bnaff=True, **kwargs
    ):
        if "abits" in kwargs:
    ...
    def _make_layer(
        self, block, conv_func, planes, blocks, stride=1, bnaff=True, **kwargs
    ):
        downsample = None
        ...
        for i in range(1, blocks):
            layers.append(
                block(conv_func, self.inplanes, planes, bnaff=bnaff, **kwargs)
            )
